...
contract PancakeSwapV3Listener is PancakeV3Pool$OnSwapFunction, DexUtils, IDexListener {
    function PancakeV3Pool$onSwapFunction(
        ...
        PancakeV3Pool$SwapFunctionOutputs memory outputs
    ) external override {
        ...
        if (factory == DexUtils.getPancakeSwapV3Factory()) {
            trade.dex = "PancakeSwapV3";
        } else if (factory == DexUtils.getDackieSwapFactory()) {
            trade.dex = "DackieSwap";
        } else if (factory == DexUtils.getSwapBasedFactory()) {
            trade.dex = "SwapBased";
        } else {
            return;
        }
